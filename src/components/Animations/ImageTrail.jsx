...
class ImageTrailVariant1 {
  constructor(container) {
    ...
    const handlePointerMove = ev => {
      ...
      console.log('Mouse moved to:', this.mousePos);
    };
...
export default function ImageTrail({ items = [], variant = 2 }) {
  ...
  useEffect(() => {
    ...
    const VariantClass = variantMap[variant] || variantMap[1];
    const instance = new VariantClass(container);
  ...
  return (
    ...
    >
      {items.map((url, i) => (
        <div 
          key={i}
          className="content__img w-[210px] aspect-[1.1] rounded-[15px] absolute top-0 left-0 opacity-0 overflow-hidden [will-change:transform,filter]"
        >
          <div
